...
| مقالات بلاگ            | ۵۴ مقاله                              | ⚠️ ۴۱ تاریخ یکسان |
| تست‌ها                 | ۸۵۷+ — همه PASS                       | ✅                |
| صفحات SSG              | ۲۳۵+ صفحه                             | ✅                |
...
| صفحه اصلی              | قهرمان "حرفه‌ای" نه "رایگان"          | ❌                |
| جستجو در صفحه اصلی     | ندارد                                 | ❌                |
| دسته‌بندی در صفحه اصلی | فقط ۶ از ۱۰                           | ⚠️                |
| فرم تماس               | فقط ذخیره در مرورگر                   | ❌                |
| قیمت‌گذاری             | بدون پلن رایگان قابل مشاهده           | ❌                |
...
## فاز ۲ — Measurement Foundation (پایه اندازه‌گیری)

**هدف:** بدون داده تبدیل، بهینه‌سازی‌ها حدسی هستند. این فاز زیربنای رشد داده‌محور است.

### ۲.۱ Google Analytics با حالت رضایت

- [ ] اضافه کردن GA4 tag با consent mode v2
- [ ] حفظ analytics خودمیزبان برای گزارش‌دهی privacy-first
- [ ] اطمینان از اینکه GA4 فقط بعد از رضایت کاربر فعال شود
- [ ] تنظیم رویدادهای اصلی: `tool_use`, `export_attempt`, `checkout_start`, `payment_success`
- [ ] تنظیم funnel: `page_view` → `tool_open` → `tool_use` → `export` → `checkout` → `payment`
...
## فاز ۴ — SEO Dominance (سلطه بر سئو)

**هدف:** نتیجه #1 گوگل برای هر عبارت جستجوی ابزار فارسی.

### ۴.۱ رفع مشکل تاریخ بلاگ

**وضعیت فعلی:** ۴۱ از ۵۴ مقاله تاریخ ۲۰۲۶-۰۶-۲۶ دارند

- [ ] پراکنده‌سازی تاریخ انتشار: گسترش ۴۱ مقاله در ۴-۶ هفته
- [ ] اضافه کردن `datePublished` و `dateModified` schema به هر مقاله
- [ ] اطمینان از تاریخ‌های واقعی و منطقی
...
## فاز ۵ — Revenue Optimization (بهینه‌سازی درآمد)

**هدف:** درآمد بدون آسیب رساندن به برند ابزار رایگان.

### ۵.۱ ساده‌سازی قیمت‌گذاری

**وضعیت فعلی (`PricingContent.tsx`):** مدل اعتباری پیچیده — "هر خروجی = ۱ اعتبار"

- [ ] تغییر زبان: "خروجی نامحدود" به جای "X اعتبار"
- [ ] اضافه کردن پلن "رایگان" قابل مشاهده در صفحه قیمت‌گذاری
- [ ] CTA رایگان: "شروع کنید — بدون ثبت‌نام"
- [ ] نمایش قیمت ماهانه معادل برای پلن سالانه (مثلاً "۷۴,۰۰۰ تومان / ماه")
- [ ] اضافه کردن "تضمین بازگشت وجه ۷ روزه" (اگر امکان‌پذیر است)
...
## فاز ۷ — Accessibility & Quality (دسترسی‌پذیری و کیفیت)
...
## فاز ۸ — Ecosystem & Growth (اکوسیستم و رشد)

**هدف:** فراتر از وب — حضور در پلتفرم‌هایی که کاربران ایرانی هستند.

### ۸.۱ Telegram Bot

- [ ] Bot برای دسترسی سریع به ابزارها: `/loan`, `/salary`, `/date`
- [ ] اشتراک‌گذاری نتایج در گروه‌ها
- [ ] اعلان‌های محتوای جدید

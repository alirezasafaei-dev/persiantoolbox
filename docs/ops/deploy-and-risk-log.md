...
**Deployed:** NO (local fix prepared; production deploy still requires explicit approval)
...
- Deploy requires explicit approval, then run the mandatory production health sequence.
- After deploy, watch PM2 restart count, heap usage, and nginx logs for disappearance of `sw.js` route burst patterns.

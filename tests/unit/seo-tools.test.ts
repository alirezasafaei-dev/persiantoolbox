...
function getGraph(result: Record<string, unknown>): Array<Record<string, unknown>> {
  return (result['@graph'] ?? []) as Array<Record<string, unknown>>;
}
...
describe('buildToolJsonLd', () => {
  ...
  it('includes BreadcrumbList', () => {
    const tool = getToolByPathOrThrow('/tools/json-formatter');
    const result = buildToolJsonLd(tool);
    const types = getGraph(result).map((n) => n['@type']);
...
describe('buildTopicJsonLd', () => {
  ...
  it('returns correct structure', () => {
    ...
    expect(result['@context']).toBe('https://schema.org');
    expect(getGraph(result)).toHaveLength(2);
  });
  ...
  it('includes FAQ when provided', () => {
    const withFaq = { ...input, faq: [{ question: 'سوال؟', answer: 'جواب' }] };
...
describe('buildPillarJsonLd', () => {
  ...
  it('returns correct structure', () => {
    ...
    expect(result['@context']).toBe('https://schema.org');
    expect(getGraph(result).length).toBeGreaterThanOrEqual(2);
  });

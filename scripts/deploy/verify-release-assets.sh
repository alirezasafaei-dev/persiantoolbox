...
assert_json_status() {
  ...
  curl_safe "$BASE_URL$endpoint" > "$output"
...
if [[ -n "$EXPECTED_COMMIT" ]]; then
  ...
  curl_safe "$BASE_URL/api/version" > "$VERSION_FILE"
...
while IFS= read -r asset; do
  [[ -n "$asset" ]] || continue
  metrics="$(curl_safe --output /dev/null --write-out '%{http_code}\t%{content_type}\t%{size_download}' "$BASE_URL$asset")" || {
...
for font in \
  ...
  curl_safe --output /dev/null "$BASE_URL$font" || {

...
export default function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const getToneClasses = (tone: Toast['tone']) => {
    if (tone === 'error') {
      return 'border-[rgb(var(--color-danger-rgb)/0.4)] bg-[rgb(var(--color-danger-rgb)/0.18)] text-[var(--color-danger)]';
    }
    if (tone === 'info') {
      return 'border-[rgb(var(--color-info-rgb)/0.4)] bg-[rgb(var(--color-info-rgb)/0.18)] text-[var(--color-info)]';
    }
    return 'border-[rgb(var(--color-success-rgb)/0.4)] bg-[rgb(var(--color-success-rgb)/0.18)] text-[var(--color-success)]';
  };

  const getToneIcon = (tone: Toast['tone']) => {
    if (tone === 'error') {
      return (
        <svg
          className="h-3 w-3"

...
const DRAFT_STORAGE_KEY = 'persian-tools.contract-salon.v1';
...
export function saveDraft(draft: SalonDraft): void {
...
export function loadDrafts(): SalonDraft[] {
  if (typeof window === 'undefined') {
    return [];
  }
  try {
    ...
    return JSON.parse(raw) as SalonDraft[];
  } catch {
    return [];
  }
}

export function loadDraft(id: string): SalonDraft | undefined {
...
export function createDraftId(): string {
  return `salon_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
}

...
const DRAFT_STORAGE_KEY = 'persian-tools.contract-lease.v1';
...
export function saveDraft(draft: LeaseDraft): void {
...
export function loadDrafts(): LeaseDraft[] {
  if (typeof window === 'undefined') {
    return [];
  }
  try {
    ...
    return JSON.parse(raw) as LeaseDraft[];
  } catch {
    return [];
  }
}

export function loadDraft(id: string): LeaseDraft | undefined {

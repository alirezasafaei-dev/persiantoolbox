...
const DRAFT_STORAGE_KEY = 'persian-tools.career-drafts.v1';
...
export function loadDrafts(): ResumeDraft[] {
  if (typeof window === 'undefined') {
    return [];
  }
  try {
...
export function loadDraft(id: string): ResumeDraft | undefined {
...
export function canSaveDraft(type: CareerDocumentType): boolean {
  const count = loadDrafts().filter((d) => d.documentType === type).length;
  return count < FREE_MAX_DRAFTS;
}

export function createDraftId(): string {
  return `cdoc_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
}
